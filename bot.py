...
cursor.execute("""CREATE TABLE IF NOT EXISTS tienda (
    ...
    rol_id INTEGER DEFAULT 0, dinero_efecto INTEGER DEFAULT 0, es_seguro INTEGER DEFAULT 0
)""")
...
cursor.execute("""CREATE TABLE IF NOT EXISTS seguros (
    guild_id INTEGER, user_id INTEGER, cantidad INTEGER DEFAULT 0,
    PRIMARY KEY (guild_id, user_id)
)""")
...
cursor.execute("""CREATE TABLE IF NOT EXISTS config (
    ...
    cooldown_ajedrez INTEGER DEFAULT 60,
...
_MIGRACIONES = [
    ...
    ("config", "cooldown_ajedrez", "INTEGER DEFAULT 60"),
...
CONFIG_FIELDS_ECONOMIA = {
    ...
    "cooldown_slut": ("int", "Cooldown de !slut en segundos"),
    "slut_chance": ("float", "Probabilidad de éxito de !slut (0 a 1)"),
    "slut_win_min": ("int", "Ganancia mínima si !slut sale bien"),
    "slut_win_max": ("int", "Ganancia máxima si !slut sale bien"),
    "slut_loss_min": ("int", "Pérdida mínima si !slut sale mal"),
    "slut_loss_max": ("int", "Pérdida máxima si !slut sale mal"),
}
...
CONFIG_FIELDS_CASINO = {
    ...
    "cooldown_ajedrez": ("int", "Cooldown de /ajedrez en segundos (solo aplica si se juega con apuesta)"),
}
...
MAPA_COOLDOWNS = {
    ...
    "ajedrez": "cooldown_ajedrez", "chess": "cooldown_ajedrez",
}
...
def get_tienda(guild_id: int):
    cursor.execute("""SELECT id, nombre, precio, descripcion, emoji, usable, imagen,
                       rol_id, dinero_efecto, es_seguro FROM tienda WHERE guild_id=? ORDER BY precio ASC""", (guild_id,))
    return cursor.fetchall()

def get_item_tienda(guild_id: int, nombre: str):
    cursor.execute("""SELECT id, nombre, precio, descripcion, emoji, usable, mensaje_uso, imagen,
                       rol_id, dinero_efecto, es_seguro FROM tienda WHERE guild_id=? AND LOWER(nombre)=LOWER(?)""", (guild_id, nombre))
...
def get_seguros(guild_id: int, user_id: int) -> int:
    cursor.execute("SELECT cantidad FROM seguros WHERE guild_id=? AND user_id=?", (guild_id, user_id))
    fila = cursor.fetchone()
    return fila[0] if fila else 0

def add_seguro(guild_id: int, user_id: int, cantidad: int = 1):
    cursor.execute("""INSERT INTO seguros (guild_id, user_id, cantidad) VALUES (?,?,?)
        ON CONFLICT(guild_id, user_id) DO UPDATE SET cantidad = cantidad + excluded.cantidad""",
        (guild_id, user_id, cantidad))
    db.commit()

def consumir_seguro(guild_id: int, user_id: int):
    cursor.execute("UPDATE seguros SET cantidad = MAX(cantidad - 1, 0) WHERE guild_id=? AND user_id=?", (guild_id, user_id))
    db.commit()
...
class ComprarButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        _id, nombre_real, precio, descripcion, emoji, usable, mensaje_uso, imagen, rol_id, dinero_efecto, es_seguro = item
...
class TiendaLayoutView(discord.ui.LayoutView):
    ...
    def __init__(self, guild_id: int, items):
        ...
        for idx, (_id, nombre, precio, descripcion, emoji, usable, imagen, rol_id, dinero_efecto, es_seguro) in enumerate(items):
            etiquetas = []
            if usable:
...
def _carta_ansi(carta: str) -> str:
    """Colorea una carta con códigos ANSI (rojo para ♥/♦, blanco para ♠/♣) para que
    se vea como en un bloque de código de Discord, similar a una carta física."""
    palo = carta[-1]
    color = "31" if palo in ("♥", "♦") else "37"
    return f"\u001b[{color}m{carta}\u001b[0m"

def render_mano_ansi(cartas, ocultar_segunda: bool = False) -> str:
    """Renderiza una mano dentro de un bloque ```ansi``` para que Discord la pinte con colores."""
    partes = []
    for i, c in enumerate(cartas):
        if ocultar_segunda and i == 1:
            partes.append("\u001b[34m🂠\u001b[0m")
        else:
            partes.append(_carta_ansi(c))
    return "```ansi\n" + "   ".join(partes) + "\n```"

# ─────────────────────────────────────────
# AJEDREZ
# ─────────────────────────────────────────
AJEDREZ_JOIN_SECONDS = 20
AJEDREZ_TURNO_SEGUNDOS = 180  # tiempo máximo por turno antes de perder por abandono
AJEDREZ_PROFUNDIDAD_IA = 2
...
def evaluar_tablero_ajedrez(board: chess.Board) -> float:
    if board.is_checkmate():
        return -99999 if board.turn == chess.WHITE else 99999
    if board.is_stalemate() or board.is_insufficient_material():
        return 0
    valor = 0
    for tipo, val in PIEZAS_VALOR.items():
        valor += len(board.pieces(tipo, chess.WHITE)) * val
        valor -= len(board.pieces(tipo, chess.BLACK)) * val
    return valor

def _negamax(board: chess.Board, profundidad: int, alpha: float, beta: float, color: int) -> float:
    if profundidad == 0 or board.is_game_over():
        return color * evaluar_tablero_ajedrez(board)
    mejor = -math.inf
    for mov in board.legal_moves:
        board.push(mov)
        valor = -_negamax(board, profundidad - 1, -beta, -alpha, -color)
        board.pop()
...
def elegir_movimiento_ia(board: chess.Board, profundidad: int = AJEDREZ_PROFUNDIDAD_IA) -> chess.Move:
    """IA simple: minimax con poda alfa-beta y evaluación por material (con algo de azar entre empates)."""
    color = 1 if board.turn == chess.WHITE else -1
    mejor_valor = -math.inf
    mejores = []
    for mov in board.legal_moves:
        board.push(mov)
        valor = -_negamax(board, profundidad - 1, -math.inf, math.inf, -color)
        board.pop()
...
def render_tablero_ajedrez(board: chess.Board) -> str:
    return "```\n" + board.unicode(borders=True) + "\n```"

def resultado_ajedrez_texto(board: chess.Board) -> str:
    if board.is_checkmate():
        ganador = "Negras ⚫" if board.turn == chess.WHITE else "Blancas ⚪"
        return f"♟️ ¡Jaque mate! Ganan las {ganador}"
    if board.is_stalemate():
        return "🤝 Tablas por ahogado (stalemate)"
    if board.is_insufficient_material():
        return "🤝 Tablas por material insuficiente"
    if board.is_seventyfive_moves():
        return "🤝 Tablas por regla de 75 movimientos"
...
class AjedrezMoverModal(discord.ui.Modal, title="Tu movimiento"):
    """Fallback por si alguien prefiere escribir el movimiento en vez de usar los menús."""
    movimiento = discord.ui.TextInput(
        label="Movimiento (SAN o UCI)",
        placeholder="Ej: e4, Cf3, Dxe5, o e2e4",
        max_length=10,
    )

    def __init__(self, view: "AjedrezMoveView"):
        super().__init__()
        self.view_ref = view

    async def on_submit(self, interaction: discord.Interaction):
        view = self.view_ref
        texto = self.movimiento.value.strip()
        board = view.board
        mov = None
        try:
            mov = board.parse_san(texto)
        except Exception:
            try:
                candidato = chess.Move.from_uci(texto.lower())
                if candidato in board.legal_moves:
                    mov = candidato
            except Exception:
                mov = None
        if mov is None:
            return await interaction.response.send_message(
                "❌ Ese movimiento no es válido we, prueba con notación SAN (ej: `e4`, `Cf3`, `Dxe5`) o UCI (ej: `e2e4`)",
                ephemeral=True)
        await view._ejecutar_movimiento(interaction, mov)

class AjedrezOrigenSelect(discord.ui.Select):
    """Primer paso: elegir qué pieza mover."""
    def __init__(self, view: "AjedrezMoveView"):
        self.view_ref = view
        board = view.board
        origenes = {}
        for mov in board.legal_moves:
            origenes.setdefault(mov.from_square, []).append(mov)
        options = []
        for square in sorted(origenes.keys()):
            pieza = board.piece_at(square)
            options.append(discord.SelectOption(
                label=f"{chess.square_name(square)} — {PIEZA_NOMBRE[pieza.piece_type]}",
                value=str(square),
                emoji=PIEZA_EMOJI[(pieza.piece_type, pieza.color)],
                description=f"{len(origenes[square])} movimiento(s) posible(s)",
            ))
        super().__init__(placeholder="1️⃣ Elige la pieza que quieres mover...", options=options[:25], min_values=1, max_values=1, row=0)

    async def callback(self, interaction: discord.Interaction):
        await self.view_ref.mostrar_destinos(interaction, int(self.values[0]))

class AjedrezDestinoSelect(discord.ui.Select):
    """Segundo paso: elegir a qué casilla mover la pieza ya seleccionada."""
    def __init__(self, view: "AjedrezMoveView", origen: int):
        self.view_ref = view
        self.origen = origen
        board = view.board
        destinos = {}
        for mov in board.legal_moves:
            if mov.from_square == origen:
                destinos.setdefault(mov.to_square, []).append(mov)
        options = []
        for square in sorted(destinos.keys()):
            movs = destinos[square]
            es_captura = board.is_capture(movs[0])
            es_promocion = any(m.promotion for m in movs)
            etiquetas_extra = []
            if es_captura:
                etiquetas_extra.append("captura")
            if es_promocion:
                etiquetas_extra.append("promoción")
            options.append(discord.SelectOption(
                label=chess.square_name(square),
                value=chess.square_name(square),
                emoji="⚔️" if es_captura else "➡️",
                description=", ".join(etiquetas_extra) if etiquetas_extra else None,
            ))
        super().__init__(placeholder="2️⃣ Elige la casilla destino...", options=options[:25], min_values=1, max_values=1, row=0)

    async def callback(self, interaction: discord.Interaction):
        await self.view_ref.confirmar_movimiento(interaction, self.origen, chess.parse_square(self.values[0]))

class AjedrezMoveView(discord.ui.View):
    """Interfaz de botones/menús para mover: 1) elegir pieza, 2) elegir destino (con botones de
    promoción si aplica). Incluye 'Volver', 'Rendirse' y un botón para escribir el movimiento a mano."""
    def __init__(self, board: chess.Board, jugador_id: int):
        super().__init__(timeout=AJEDREZ_TURNO_SEGUNDOS)
        self.board = board
    ...
    def _boton(self, label, style, emoji, row, callback):
        boton = discord.ui.Button(label=label, style=style, emoji=emoji, row=row)
        boton.callback = callback
        return boton

    def _construir_paso_origen(self):
        self.clear_items()
        self.origen = None
        self.add_item(AjedrezOrigenSelect(self))
        self.add_item(self._boton("Rendirse", discord.ButtonStyle.danger, "🏳️", 1, self._on_rendirse))
        self.add_item(self._boton("Escribir movimiento", discord.ButtonStyle.secondary, "⌨️", 1, self._on_escribir))

    async def mostrar_destinos(self, interaction: discord.Interaction, origen: int):
        self.origen = origen
        self.clear_items()
        self.add_item(AjedrezDestinoSelect(self, origen))
        self.add_item(self._boton("Volver", discord.ButtonStyle.secondary, "↩️", 1, self._on_volver))
        self.add_item(self._boton("Rendirse", discord.ButtonStyle.danger, "🏳️", 1, self._on_rendirse))
        pieza = self.board.piece_at(origen)
        contenido = f"{PIEZA_EMOJI[(pieza.piece_type, pieza.color)]} Moviendo **{PIEZA_NOMBRE[pieza.piece_type]}** desde `{chess.square_name(origen)}` — elige destino:"
        await interaction.response.edit_message(content=contenido, view=self)

    async def confirmar_movimiento(self, interaction: discord.Interaction, origen: int, destino: int):
        candidatos = [m for m in self.board.legal_moves if m.from_square == origen and m.to_square == destino]
        if len(candidatos) == 1:
            return await self._ejecutar_movimiento(interaction, candidatos[0])
        # hay varias opciones = movimiento de promoción, hay que elegir la pieza
        self.clear_items()
        for tipo, nombre in ((chess.QUEEN, "Dama"), (chess.ROOK, "Torre"), (chess.BISHOP, "Alfil"), (chess.KNIGHT, "Caballo")):
            emoji = PIEZA_EMOJI[(tipo, self.board.turn)]
            async def cb(interaction2: discord.Interaction, tipo=tipo):
                mov = chess.Move(origen, destino, promotion=tipo)
                await self._ejecutar_movimiento(interaction2, mov)
            self.add_item(self._boton(nombre, discord.ButtonStyle.primary, emoji, 0, cb))
        await interaction.response.edit_message(content="👑 ¿A qué pieza quieres promocionar tu peón?", view=self)

    async def _on_volver(self, interaction: discord.Interaction):
        self._construir_paso_origen()
        await interaction.response.edit_message(content=None, view=self)

    async def _on_escribir(self, interaction: discord.Interaction):
        await interaction.response.send_modal(AjedrezMoverModal(self))

    async def _on_rendirse(self, interaction: discord.Interaction):
        self.terminado = True
        self.abandono = True
    ...
    async def _ejecutar_movimiento(self, interaction: discord.Interaction, mov: chess.Move):
        self.board.push(mov)
        self.terminado = True
        self.clear_items()
        try:
...
class AjedrezJoinView(discord.ui.View):
    """Máximo 2 jugadores (el retador + 1 oponente). Si nadie se une, juega contra la IA."""
    def __init__(self, retador: discord.Member, apuesta: int = 0):
        super().__init__(timeout=AJEDREZ_JOIN_SECONDS + 10)
    ...
    async def unirse_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.cerrado:
            return await interaction.response.send_message("Ya cerró la inscripción we", ephemeral=True)
        if interaction.user.id == self.retador.id:
            return await interaction.response.send_message("No puedes jugar contra ti mismo we", ephemeral=True)
        if self.oponente is not None:
            return await interaction.response.send_message("Esta partida es solo para 2 personas y ya está completa we", ephemeral=True)
        if self.apuesta > 0:
            guild_id = interaction.guild.id
            saldo = get_balance(guild_id, interaction.user.id)
            if saldo < self.apuesta:
                return await interaction.response.send_message(
                    f"❌ No tienes esa plata we, tu balance es **{format_dinero(guild_id, saldo)}** y la apuesta es **{format_dinero(guild_id, self.apuesta)}**",
                    ephemeral=True)
        self.oponente = interaction.user
        self.cerrado = True
        for item in self.children:
            item.disabled = True
        try:
...
def construir_embed_ajedrez_join(view: AjedrezJoinView, restante: int) -> discord.Embed:
    guild_id = view.retador.guild.id
    desc = f"{view.retador.mention} quiere jugar ajedrez ♟️\n"
    if view.apuesta > 0:
        desc += f"💰 Apuesta: **{format_dinero(guild_id, view.apuesta)}**\n\n"
    else:
        desc += "Sin apuesta, solo por diversión.\n\n"
    if view.oponente:
        desc += f"Oponente: {view.oponente.mention}"
    else:
        desc += "Nadie se ha unido todavía. Si nadie entra, jugará contra la IA 🤖"
    emb = discord.Embed(title="♟️ Ajedrez — ¡Únete!", description=desc, color=0xF1C40F)
    emb.set_footer(text=f"⏳ Se cierra la inscripción en {restante}s — pulsa Unirse para participar (máx. 2 jugadores)")
    return emb

async def ejecutar_ajedrez_join_window(view: AjedrezJoinView, msg: discord.Message):
    restante = AJEDREZ_JOIN_SECONDS
    while restante > 0 and not view.cerrado:
        espera = min(3, restante)
        await asyncio.sleep(espera)
        restante -= espera
        try:
            await msg.edit(embed=construir_embed_ajedrez_join(view, restante))
        except discord.HTTPException:
            pass
    view.cerrado = True
...
class EconomiaCog(commands.Cog):
    ...
    async def leaderboard(self, ctx: commands.Context, flag: str = None):
        ...
        cursor.execute(f"SELECT user_id, {columna} AS total FROM economia WHERE guild_id=? ORDER BY total DESC LIMIT 10", (ctx.guild.id,))
    ...
    async def useitem_slash(self, interaction: discord.Interaction, nombre: str):
        ...
        rol_id = info[8] if info else 0
        dinero_efecto = info[9] if info else 0
        es_seguro = info[10] if info else 0
        if not usable:
...
async def autocomplete_items_tienda(interaction: discord.Interaction, current: str):
    ...
    current = current.lower()
    return [app_commands.Choice(name=nombre, value=nombre) for _id, nombre, precio, descripcion, emoji, usable, imagen, rol_id, dinero_efecto, es_seguro in items if current in nombre.lower()][:25]
...
class ConfigCog(commands.Cog):
    ...
    async def additem_slash(self, interaction: discord.Interaction, nombre: str, precio: int, descripcion: str = "", imagen: str = ""):
        ...
        await interaction.response.send_message(
            ...
            f"Usa `/edititem` con campo `emoji`, `usable`, `imagen`, `rol`, `dinero` o `seguro` para personalizarlo.")
    ...
    async def edititem_slash(self, interaction: discord.Interaction, nombre: str, campo: app_commands.Choice[str], valor: str):
        ...
        if campo_val == "nombre":
            ...
            if valor.lower() in ("no", "ninguno", "quitar", "0"):
                cursor.execute("UPDATE tienda SET rol_id=0 WHERE id=?", (_id,))
            else:
                ...
                cursor.execute("UPDATE tienda SET rol_id=? WHERE id=?", (rol_id_val, _id))
            ...
            cursor.execute("UPDATE tienda SET dinero_efecto=? WHERE id=?", (dinero_val, _id))
        elif campo_val == "seguro":
            es_seguro_val = 1 if valor.lower() in ("si", "sí", "true", "1", "yes") else 0
            cursor.execute("UPDATE tienda SET es_seguro=? WHERE id=?", (es_seguro_val, _id))
        db.commit()
...
class CasinoCog(commands.Cog):
    ...
    async def _resolver_blackjack(self, guild_id, msg, member, mano, mazo, dealer, data):
        ...
        if valor_mano(mano) == 21:
            ...
            emb = turno.construir_embed(member)
            try:
        ...
        lineas_final = [
            ...
            "**Your Hand**",
            render_mano_ansi(mano),
            f"Value: {valor_jugador}",
            "",
            "**Dealer Hand**",
            render_mano_ansi(dealer),
            f"Value: {valor_dealer}",
        ]
        emb_final = discord.Embed(title="🃏 Resultado final", description="\n".join(lineas_final), color=0x2ECC71)
        await msg.edit(embed=emb_final, view=None)
...
class TurnoBlackjackView(discord.ui.View):
    ...
    def construir_embed(self, member: discord.Member, resultado: str = None) -> discord.Embed:
        color = 0x3498DB
        ...
        desc += f"**Your Hand**\n{render_mano_ansi(self.mano)}\nValue: {valor_mano(self.mano)}\n\n"

        oculto = resultado is None
        desc += f"**Dealer Hand**\n{render_mano_ansi(self.dealer, ocultar_segunda=oculto)}\n"
        if oculto:
            desc += f"Value: {valor_mano([self.dealer[0]])}\n\n"
        else:
            desc += f"Value: {valor_mano(self.dealer)}\n\n"
    ...
    async def hit(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if valor_mano(self.mano) > 21:
            ...
            for item in self.children:
                item.disabled = True
            emb = self.construir_embed(interaction.user, resultado="bust")
            await interaction.response.edit_message(embed=emb, view=self)
            self.evento.set()
        else:
            emb = self.construir_embed(interaction.user, resultado=None)
            await interaction.response.edit_message(embed=emb, view=self)
    ...
    async def stand(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for item in self.children:
            item.disabled = True
        emb = self.construir_embed(interaction.user, resultado="stand")
        await interaction.response.edit_message(embed=emb, view=self)
        self.evento.set()
    ...
    async def doblar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        emb = self.construir_embed(interaction.user, resultado=resultado)
        await interaction.response.edit_message(embed=emb, view=self)
        self.evento.set()
...
# COG AJEDREZ
# ─────────────────────────────────────────
class AjedrezCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self._tareas_activas: set[asyncio.Task] = set()

    @app_commands.command(name="ajedrez", description="Juega ajedrez — invita a otra persona o juega contra la IA (máx. 2 jugadores)")
    @app_commands.describe(apuesta="Opcional: cuánto quieres apostar (o 'all'). Si lo dejas vacío, se juega sin apuesta")
    async def ajedrez_slash(self, interaction: discord.Interaction, apuesta: str = None):
        guild_id = interaction.guild.id
        apuesta_valor = 0
        if apuesta is not None and apuesta.strip() != "":
            cfg = get_config(guild_id)
            restante = tiempo_restante(get_cooldown_generic(guild_id, interaction.user.id, "ajedrez"), cfg["cooldown_ajedrez"])
            if restante:
                return await interaction.response.send_message(f"⏳ Espera **{formatear_tiempo(restante)}** para volver a jugar con apuesta", ephemeral=True)
            saldo = get_balance(guild_id, interaction.user.id)
            apuesta_valor = parse_cantidad(apuesta.strip(), saldo)
            if apuesta_valor is None:
                return await interaction.response.send_message("❌ La apuesta tiene que ser un número o `all` we", ephemeral=True)
            ok, error = validar_apuesta_valores(cfg, apuesta_valor)
            if not ok:
                return await interaction.response.send_message(f"❌ {error}", ephemeral=True)
            if saldo < apuesta_valor:
                return await interaction.response.send_message(f"❌ No tienes esa plata we, tu balance es **{format_dinero(guild_id, saldo)}**", ephemeral=True)

        view = AjedrezJoinView(interaction.user, apuesta_valor)
        await interaction.response.send_message(embed=construir_embed_ajedrez_join(view, AJEDREZ_JOIN_SECONDS), view=view)
        msg = await interaction.original_response()

        # La partida puede durar varios minutos (ventana de unirse + turnos). No la dejamos
        # colgando de la task interna de discord.py (que solo tiene una referencia débil y
        # puede ser recolectada por el GC a mitad de partida) — la corremos como una task
        # propia que este Cog referencia mientras dure, para que nunca se quede "trabada".
        tarea = asyncio.create_task(self._correr_partida(msg, view))
        self._tareas_activas.add(tarea)
        tarea.add_done_callback(self._tareas_activas.discard)

    async def _correr_partida(self, msg: discord.Message, view: AjedrezJoinView):
        try:
            await ejecutar_ajedrez_join_window(view, msg)
            await self._resolver_ajedrez(msg, view)
        except Exception:
            logging.exception("Error corriendo una partida de /ajedrez")
            guild_id = view.retador.guild.id
            if view.apuesta > 0:
                # si ya se habían descontado las apuestas, se devuelven para no dejar a nadie sin su plata
                try:
                    modificar_balance(guild_id, view.retador.id, view.apuesta)
                    if view.oponente is not None:
                        modificar_balance(guild_id, view.oponente.id, view.apuesta)
                except Exception:
                    logging.exception("Error devolviendo apuestas tras fallo en /ajedrez")
            try:
                await msg.edit(content=None, embed=discord.Embed(
                    title="♟️ Ajedrez",
                    description="❌ Ocurrió un error inesperado we, se canceló la partida" + (" y se devolvieron las apuestas" if view.apuesta > 0 else ""),
                    color=0xE74C3C), view=None)
            except discord.HTTPException:
                pass

    async def _resolver_ajedrez(self, msg: discord.Message, view: AjedrezJoinView):
        guild_id = view.retador.guild.id
        blancas = view.retador
        negras = view.oponente  # None => juega contra la IA
        apuesta = view.apuesta

        # revalida y descuenta las apuestas justo antes de empezar
        if apuesta > 0:
            if get_balance(guild_id, blancas.id) < apuesta:
                emb = discord.Embed(title="♟️ Ajedrez", description=f"{blancas.mention} ya no tiene la plata suficiente we, se canceló la partida", color=0xE74C3C)
                return await msg.edit(content=None, embed=emb, view=None)
            if negras is not None and get_balance(guild_id, negras.id) < apuesta:
                emb = discord.Embed(title="♟️ Ajedrez", description=f"{negras.mention} ya no tiene la plata suficiente we, se canceló la partida", color=0xE74C3C)
                return await msg.edit(content=None, embed=emb, view=None)
            modificar_balance(guild_id, blancas.id, -apuesta)
            set_cooldown_generic(guild_id, blancas.id, "ajedrez")
            if negras is not None:
                modificar_balance(guild_id, negras.id, -apuesta)
                set_cooldown_generic(guild_id, negras.id, "ajedrez")

        board = chess.Board()

        def encabezado() -> str:
            desc = f"⚪ Blancas: {blancas.mention}\n⚫ Negras: {negras.mention if negras else '🤖 IA'}\n"
            if apuesta > 0:
                desc += f"💰 Apuesta: **{format_dinero(guild_id, apuesta)}**\n"
            return desc + "\n"

        def pagar_ganador(ganador):
            """ganador es un discord.Member (humano) o None (gana la IA / house)."""
            if apuesta <= 0:
                return
            if ganador is not None:
                modificar_balance(guild_id, ganador.id, apuesta * 2)
            # si gana la IA, la casa se queda con lo ya descontado, no hay que hacer nada más
        ...
        while not board.is_game_over():
            ...
            if not es_turno_blancas and negras is None:
                # turno de la IA
                emb = discord.Embed(title="♟️ Ajedrez", description=encabezado() + render_tablero_ajedrez(board) + "\n🤖 La IA está pensando...", color=0x3498DB)
                try:
                    await msg.edit(content=None, embed=emb, view=None)
                except discord.HTTPException:
                    pass
                await asyncio.sleep(1.5)
                mov = elegir_movimiento_ia(board)
                board.push(mov)
                continue

            jugador_member = blancas if es_turno_blancas else negras
            turno_txt = "Blancas ⚪" if es_turno_blancas else "Negras ⚫"
            move_view = AjedrezMoveView(board, jugador_member.id)
            desc = encabezado() + render_tablero_ajedrez(board) + f"\nTurno de **{turno_txt}**: {jugador_member.mention}"
            if board.is_check():
                desc += "\n⚠️ ¡Jaque!"
            emb = discord.Embed(title="♟️ Ajedrez", description=desc, color=0x3498DB)
            try:
                await msg.edit(content=None, embed=emb, view=move_view)
            except discord.HTTPException:
                pass
            await move_view.evento.wait()

            if move_view.abandono:
                ganador = negras if es_turno_blancas else blancas
                pagar_ganador(ganador)
                ganador_txt = ganador.mention if ganador else "🤖 IA"
                extra = f"\n💰 {ganador_txt} se gana **{format_dinero(guild_id, apuesta * 2)}**" if apuesta > 0 and ganador else ""
                desc_final = encabezado() + f"🏳️ {jugador_member.mention} abandonó la partida.\n**Gana {ganador_txt}**" + extra
                emb_final = discord.Embed(title="♟️ Ajedrez — Fin de la partida", description=desc_final, color=0xE74C3C)
                try:
                    await msg.edit(content=None, embed=emb_final, view=None)
                except discord.HTTPException:
                    pass
                return

        resultado_txt = resultado_ajedrez_texto(board)
        extra = ""
        if board.is_checkmate():
            # el bando que sigue en turno es el que perdió (está en jaque mate)
            ganador = negras if board.turn == chess.WHITE else blancas
            pagar_ganador(ganador)
            if apuesta > 0 and ganador is not None:
                extra = f"\n💰 {ganador.mention} se gana **{format_dinero(guild_id, apuesta * 2)}**"
        else:
            pagar_empate()
            if apuesta > 0:
                extra = f"\n🤝 Cada quien recupera su apuesta de **{format_dinero(guild_id, apuesta)}**"

        desc = encabezado() + render_tablero_ajedrez(board) + f"\n\n{resultado_txt}{extra}"
        emb_final = discord.Embed(title="♟️ Ajedrez — Fin de la partida", description=desc, color=0x2ECC71)
        try:
            await msg.edit(content=None, embed=emb_final, view=None)
        except discord.HTTPException:
            pass
...
class HelpCog(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="♟️ Ajedrez", value=(
            "`/ajedrez <apuesta>` — Abre un botón **Unirse ♟️** por 20s para que otra persona entre (máximo 2 jugadores en total). "
            "Si nadie se une, juegas contra la IA 🤖. La apuesta es opcional (déjala vacía para jugar gratis, o usa `all`); "
            "quien gana se lleva ambas apuestas, en empate cada quien recupera la suya.\n"
            "Para mover: elige tu pieza en el primer menú, luego la casilla destino en el segundo (si es un peón que corona, te pregunta a qué pieza). "
            "También hay botones de **Volver**, **Rendirse** y **Escribir movimiento** (por si prefieres notación SAN/UCI como `e4` o `e2e4`)."
        ), inline=False)
        ...
        if staff:
            ...
            embed.add_field(name="🛒 Tienda Staff 🔒", value=(
                ...
                "`/edititem <nombre> <campo> <valor>` — Edita nombre/precio/descripción/emoji/usable/mensaje/imagen/rol/dinero/seguro\n"
...
async def on_ready():
    ...
    await bot.add_cog(AjedrezCog(bot))
