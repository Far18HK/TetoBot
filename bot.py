...
class ComprarButton(discord.ui.Button):
    def __init__(self, nombre: str, precio: int, emoji_str: str, guild_id: int):
        label = f"{nombre} — {precio:,}"
        super().__init__(label=label[:80], style=discord.ButtonStyle.success, emoji=parse_emoji_boton(emoji_str))
...
class TiendaView(discord.ui.View):
    def __init__(self, guild_id: int, items):
        ...
        # Discord permite máximo 25 componentes por mensaje (5 filas x 5)
        for _id, nombre, precio, descripcion, emoji, usable in items[:25]:
            self.add_item(ComprarButton(nombre, precio, emoji, guild_id))

def construir_embed_tienda(guild_id: int, items) -> discord.Embed:
    embed = discord.Embed(title="🛒 Tienda", color=0x3498DB,
                           description="Pulsa un botón para comprar el item al instante.")
    for _id, nombre, precio, descripcion, emoji, usable in items:
        etiqueta = "\n*Usable con `!useitem`*" if usable else ""
        embed.add_field(name=f"{emoji} {nombre} — {format_dinero(guild_id, precio)}", value=(descripcion or "\u200b") + etiqueta, inline=False)
    if len(items) > 25:
        embed.set_footer(text="⚠️ Solo se muestran botones para los primeros 25 items")
    return embed
...
class ApuestaModal(discord.ui.Modal):
    def __init__(self, view: "JoinView"):
        ...
        self.view_ref = view
        self.apuesta_input = discord.ui.TextInput(label="¿Cuánto quieres apostar?", placeholder="Ej: 100", max_length=10)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.response.send_message("❌ La apuesta tiene que ser un número we", ephemeral=True)
...
class EconomiaCog(commands.Cog):
    ...
    async def tienda_cmd(self, ctx: commands.Context):
        ...
        embed = construir_embed_tienda(ctx.guild.id, items)
        view = TiendaView(ctx.guild.id, items)
...
async def autocomplete_items_tienda(interaction: discord.Interaction, current: str):
    ...
    return [app_commands.Choice(name=nombre, value=nombre) for _id, nombre, precio, descripcion, emoji, usable in items if current in nombre.lower()][:25]
...
class ConfigCog(commands.Cog):
    ...
    # ── PLATA (STAFF) ──
    @app_commands.command(name="add$", description="Agrega plata a un usuario")
    ...
    @app_commands.command(name="remove$", description="Quita plata a un usuario")
    ...
    async def additem_slash(self, interaction: discord.Interaction, nombre: str, precio: int, descripcion: str = ""):
        ...
        cursor.execute("INSERT INTO tienda (guild_id, nombre, precio, descripcion, emoji, usable, mensaje_uso) VALUES (?,?,?,?,?,?,?)",
                       (interaction.guild.id, nombre, precio, descripcion, "📦", 0, ""))
        db.commit()
        await interaction.response.send_message(
            f"✅ Agregado **{nombre}** a la tienda por **{format_dinero(interaction.guild.id, precio)}**\n"
            f"Usa `/edititem` con campo `emoji` o `usable` para personalizarlo.")
...
class CasinoCog(commands.Cog):
    ...
    async def _chequeo_inicial(self, interaction: discord.Interaction, juego_tipo: str, apuesta: int):
        """Valida cooldown, apuesta y saldo del que inicia la ronda. Devuelve (cfg, ok)."""
        guild_id = interaction.guild.id
        cfg = get_config(guild_id)
    ...
    @app_commands.describe(apuesta="Cuánto quieres apostar")
    async def slots_slash(self, interaction: discord.Interaction, apuesta: int):
        cfg, ok = await self._chequeo_inicial(interaction, "slots", apuesta)
        if not ok:
            return
        guild_id = interaction.guild.id
    ...
    @app_commands.describe(apuesta="Cuánto quieres apostar", color="rojo, negro o verde")
    ...
    async def ruleta_slash(self, interaction: discord.Interaction, apuesta: int, color: app_commands.Choice[str]):
        cfg, ok = await self._chequeo_inicial(interaction, "ruleta", apuesta)
        if not ok:
            return
        guild_id = interaction.guild.id
    ...
    @app_commands.describe(apuesta="Cuánto quieres apostar", lado="cara o cruz")
    @app_commands.choices(lado=[
        app_commands.Choice(name="Cara", value="cara"),
        app_commands.Choice(name="Cruz", value="cruz"),
    ])
    async def coinflip_slash(self, interaction: discord.Interaction, apuesta: int, lado: app_commands.Choice[str]):
        cfg, ok = await self._chequeo_inicial(interaction, "coinflip", apuesta)
        if not ok:
            return
        guild_id = interaction.guild.id
    ...
    @app_commands.describe(apuesta="Cuánto quieres apostar")
    async def blackjack_slash(self, interaction: discord.Interaction, apuesta: int):
        cfg, ok = await self._chequeo_inicial(interaction, "blackjack", apuesta)
        if not ok:
            return
        guild_id = interaction.guild.id
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        if staff:
            ...
            embed.add_field(name="🛒 Tienda Staff 🔒", value=(
                "`/additem <nombre> <precio> <descripcion>` — Agrega un item a la tienda\n"
