...
def construir_embeds_tienda(guild_id: int, items) -> list:
    ...
    for _id, nombre, precio, descripcion, emoji, usable, imagen in items[:MAX_ITEMS_TIENDA]:
        ...
        emb = discord.Embed(description=(descripcion or "\u200b") + etiqueta, color=0x2ECC71)
        if imagen and imagen.strip().lower().startswith(("http://", "https://")):
            emb.set_author(name=f"{emoji} {nombre}", icon_url=imagen.strip())
        else:
            emb.set_author(name=f"{emoji} {nombre}")
        embeds.append(emb)
