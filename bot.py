...
CONFIG_FIELDS_CASINO = {
    ...
    "ruleta_multi_numero": ("int", "Multiplicador de /ruleta si aciertas el número exacto (0 a 36)"),
...
class ConfigCog(commands.Cog):
    ...
    @app_commands.command(name="datos", description="Descarga la base de datos del bot (.db)")
    @is_staff_app()
    async def datos_slash(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        db.commit()  # aseguramos que todo lo pendiente esté escrito en el archivo
        if not os.path.exists(DB_PATH):
            return await interaction.followup.send("❌ No encontré el archivo de la base de datos we", ephemeral=True)
        tamano_mb = os.path.getsize(DB_PATH) / (1024 * 1024)
        limite_mb = interaction.guild.filesize_limit / (1024 * 1024)
        if tamano_mb >= limite_mb:
            return await interaction.followup.send(
                f"❌ La base de datos pesa {tamano_mb:.1f}MB y supera el límite de subida de este server ({limite_mb:.0f}MB) we",
                ephemeral=True)
        archivo = discord.File(DB_PATH, filename=os.path.basename(DB_PATH))
        await interaction.followup.send(
            content=f"📦 Acá está `{os.path.basename(DB_PATH)}` ({tamano_mb:.2f}MB). Para migrar, súbela en el nuevo host "
                    f"y apunta la variable `DB_PATH` a esa ruta.",
...
class HelpCog(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction):
        staff = isinstance(interaction.user, discord.Member) and es_staff_member(interaction.user)
        ...
        if staff:
            ...
            embed.add_field(name="⚙️ Configuración Staff 🔒", value=(
                ...
                "`/datos` — Te manda el archivo `.db` con toda la base de datos, para migrar de host fácil"
            ), inline=False)
