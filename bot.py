...
def set_config_field(guild_id: int, campo: str, valor):
    ...
    cursor.execute(f"UPDATE config SET {campo} = %s WHERE guild_id=%s", (valor, guild_id))
    db.commit()
...
def modificar_balance(guild_id: int, user_id: int, cantidad: int):
    _ensure_user(guild_id, user_id)
    cursor.execute("UPDATE economia SET balance = MAX(balance + %s, 0) WHERE guild_id=%s AND user_id=%s", (cantidad, guild_id, user_id))
    db.commit()
...
def modificar_banco(guild_id: int, user_id: int, cantidad: int):
    _ensure_user(guild_id, user_id)
    cursor.execute("UPDATE economia SET banco = MAX(banco + %s, 0) WHERE guild_id=%s AND user_id=%s", (cantidad, guild_id, user_id))
    db.commit()
...
def get_tienda(guild_id: int):
    cursor.execute("""SELECT id, nombre, precio, descripcion, emoji, usable, imagen,
                       rol_id, dinero_efecto, es_seguro FROM tienda WHERE guild_id=%s ORDER BY precio ASC""", (guild_id,))
    return cursor.fetchall()
...
def get_item_tienda(guild_id: int, nombre: str):
    cursor.execute("""SELECT id, nombre, precio, descripcion, emoji, usable, mensaje_uso, imagen,
                       rol_id, dinero_efecto, es_seguro FROM tienda WHERE guild_id=%s AND LOWER(nombre)=LOWER(%s)""", (guild_id, nombre))
...
def consumir_seguro(guild_id: int, user_id: int):
    cursor.execute("UPDATE seguros SET cantidad = MAX(cantidad - 1, 0) WHERE guild_id=%s AND user_id=%s", (guild_id, user_id))
    db.commit()
...
class ConfigCog(commands.Cog):
    ...
    async def resetconfig_slash(self, interaction: discord.Interaction):
        cursor.execute("DELETE FROM config WHERE guild_id=%s", (interaction.guild.id,))
        db.commit()
    ...
    async def additem_slash(self, interaction: discord.Interaction, nombre: str, precio: int, descripcion: str = "", imagen: str = ""):
        ...
        cursor.execute("INSERT INTO tienda (guild_id, nombre, precio, descripcion, emoji, usable, mensaje_uso, imagen) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    async def edititem_slash(self, interaction: discord.Interaction, nombre: str, campo: app_commands.Choice[str], valor: str):
        ...
        if campo_val == "nombre":
            ...
            cursor.execute("UPDATE tienda SET emoji=%s WHERE id=%s", (valor, _id))
