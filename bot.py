...
class TiendaView(discord.ui.View):
    def __init__(self, guild_id: int, items):
        ...
        # Solo mostramos botones para los items que también tienen su propio embed (ver MAX_ITEMS_TIENDA)
        for _id, nombre, precio, descripcion, emoji, usable, imagen in items[:MAX_ITEMS_TIENDA]:
            self.add_item(ComprarButton(nombre, precio, guild_id))

def construir_embeds_tienda(guild_id: int, items) -> list:
    """Arma un embed individual por item (con miniatura si tiene imagen), al estilo tarjeta de tienda."""
    embeds = []
    for _id, nombre, precio, descripcion, emoji, usable, imagen in items[:MAX_ITEMS_TIENDA]:
        etiqueta = "\n*Usable con `!useitem`*" if usable else ""
        emb = discord.Embed(title=f"{emoji} {nombre}", description=(descripcion or "\u200b") + etiqueta, color=0x2ECC71)
        emb.add_field(name="Precio", value=format_dinero(guild_id, precio))
        if imagen:
            emb.set_thumbnail(url=imagen)
        embeds.append(emb)
    if len(items) > MAX_ITEMS_TIENDA:
        embeds[-1].set_footer(text=f"⚠️ Mostrando {MAX_ITEMS_TIENDA} de {len(items)} items we")
    return embeds
...
class EconomiaCog(commands.Cog):
    ...
    async def tienda_cmd(self, ctx: commands.Context):
        ...
        embeds = construir_embeds_tienda(ctx.guild.id, items)
        view = TiendaView(ctx.guild.id, items)
        await ctx.send(content=contenido, embeds=embeds, view=view)
