    ...
    async def _resolver_blackjack(self, guild_id, msg, view):
        ...
        for uid, data in validos.items():
            if uid in resultados_naturales:
                continue
            turno = TurnoBlackjackView(uid, manos[uid], mazo, dealer, guild_id, data)
            emb = turno.construir_embed(data["member"])
            try:
        ...
        for uid, data in validos.items():
            ...
            if resultados_naturales.get(uid) == "blackjack":
                if valor_dealer == 21 and len(dealer) == 2:
                    ...
                    texto = "🤝 Empate (Blackjack). Recupera su apuesta"
                else:
                ...
                ganancia = apuesta * 2
                modificar_balance(guild_id, uid, ganancia)
                texto = f"🎉 Ganó +{format_dinero(guild_id, ganancia)}"
            elif valor_jugador == valor_dealer:
                modificar_balance(guild_id, uid, apuesta)
                texto = "🤝 Empate. Recupera su apuesta"
            else:
                texto = f"😢 Perdió -{format_dinero(guild_id, apuesta)}"
            lineas_final.append(f"**{data['member'].display_name}** — Result: {texto}")
            lineas_final.append(render_mano_ansi(mano))
            lineas_final.append(f"Value: {valor_jugador}")
            lineas_final.append("")
