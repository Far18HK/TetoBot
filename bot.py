...
class ComprarButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        _id, nombre_real, precio, descripcion, emoji, usable, mensaje_uso, imagen, rol_id, dinero_efecto, es_seguro, categoria = item
        ...
        if not items:
            ...
            for i, (tipo, data) in enumerate(bloques):
                ...
                if tipo == "header":
                    ...
                    continue
                _id, nombre, precio, descripcion, emoji, usable, imagen, rol_id, dinero_efecto, es_seguro, categoria = data
                etiquetas = []
                if usable:
...
async def autocomplete_items_tienda(interaction: discord.Interaction, current: str):
    ...
    current = current.lower()
    return [app_commands.Choice(name=nombre, value=nombre) for _id, nombre, precio, descripcion, emoji, usable, imagen, rol_id, dinero_efecto, es_seguro, categoria in items if current in nombre.lower()][:25]
...
class ConfigCog(commands.Cog):
    ...
    async def additem_slash(self, interaction: discord.Interaction, nombre: str, precio: int, descripcion: str = "", categoria: str = "General", imagen: str = ""):
        ...
        cursor.execute("INSERT INTO tienda (guild_id, nombre, precio, descripcion, emoji, usable, mensaje_uso, imagen, categoria) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        ...
        await interaction.response.send_message(
            ...
            f"Usa `/edititem` con campo `emoji`, `usable`, `imagen`, `rol`, `dinero`, `seguro` o `categoria` para personalizarlo.")
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        if staff:
            ...
            embed.add_field(name="🛒 Tienda Staff 🔒", value=(
                ...
                "`/edititem <nombre> <campo> <valor>` — Edita nombre/precio/descripción/categoria/emoji/usable/mensaje/imagen/rol/dinero/seguro\n"
            ...
            embed.add_field(name="⚙️ Configuración Staff 🔒", value=(
                "La configuración de economía y casino (cooldowns, ganancias, apuestas, moneda, etc.) "
                "ahora se edita desde el **Dashboard web**, no con comandos.\n"
